...
// ── Doble de Postgres, con la misma semantica que las tablas reales ──────────
//
// Reproduce lo que importa: el lease con caducidad (que es lo que impide que
// dos busquedas del mismo grupo corrompan el cursor) y la EWMA de las
// estadisticas. Lo demas de `pg` no hace falta para probar esto.
function crearBaseFalsa() {
    const rotaciones = new Map();
    const estadisticas = new Map();

    // La tabla plugin_search_jobs, con lo que de verdad importa probar: que el
    // trabajo sobrevive al proceso, que un huerfano acaba expirado y que lo
    // vencido se borra.
    ...
    return {
        ...
        instalar() {
            ...
            jobRepo.leer = async searchId => {
                ...
                return fila ? { ...fila, desdeBase: true } : null;
            };

            jobRepo.expirarHuerfanos = async () => {
                if (!disponible) return 0;
                let n = 0;
                const limite = Date.now() - config.pluginJobs.heartbeatTimeoutMs;
                for (const fila of trabajosPersistidos.values()) {
                    if (!['queued', 'running'].includes(fila.status)) continue;
                    if (fila.heartbeatAt !== null && fila.heartbeatAt >= limite) continue;
                    fila.status = 'expired';
            ...
            repo.adquirir = async (groupId, ordenInicial) => {
                ...
                nueva.leaseExpiresAt = ahora + config.pluginRotation.leaseMs;
            ...
            repo.guardar = async estado => {
                ...
                fila.leaseExpiresAt = Date.now() + config.pluginRotation.leaseMs;
                return true;
            };
        ...
        // Es exactamente lo que ve nuestra instancia en un despliegue con
        // varias replicas.
